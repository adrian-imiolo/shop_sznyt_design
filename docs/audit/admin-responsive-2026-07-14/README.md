...
| Revenue banner: no overflow at "over" with 5-digit total | `revenue-banner-over-375.png` — real component render: `/revenue/quarter` response patched at the fetch layer to `threshold: "over"`, `totalPln: 12345.67`, so the component's own threshold mapping produced the red container, red bar (clamped to 100%), and over message. Amounts line `12 345,67 zł / 10 813,50 zł (114.2%)` measures 214px — fits, no page scroll |
...
1. **Products table left columns were unreachable below ~900px.** The wrapper was
   `flex flex-col items-center w-full overflow-x-auto`; centering a 900px+ table in a
